...
class AffinitasBillingTest extends TestCase
{
    use RefreshDatabase;

    /** @test */
    public function admins_can_browse_to_the_affinitas_index_page()
    {
        $this->signIn();

        $this->get(route('affinitas.index'))
            ->assertOk()
            ->assertSessionHasNoErrors()
            ->assertViewIs("tools.affinitas.index")
            ->assertSee('Cobranza Affinitas');
    }
